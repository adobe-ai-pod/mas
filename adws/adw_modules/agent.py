...
SLASH_COMMAND_MODEL_MAP: Final[Dict[SlashCommand, Dict[ModelSet, str]]] = {
    ...
    "/reproduce_issue": {"base": "sonnet", "heavy": "sonnet"},
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    if request.working_dir:
        # First try playwright-specific config for review workflows
        playwright_config = os.path.join(request.working_dir, "mcp", ".mcp.playwright.json")
        if os.path.exists(playwright_config):
            cmd.extend(["--mcp-config", playwright_config])
        else:
            # Fallback to default .mcp.json
