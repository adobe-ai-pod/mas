...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    ANTHROPIC_API_KEY is optional if using `claude auth login` authentication.
    CLAUDE_CODE_PATH defaults to "claude" if not set.
    ...
    """
    # ANTHROPIC_API_KEY is optional - Claude CLI auth is used if not provided
    # CLAUDE_CODE_PATH defaults to "claude" if not set
    ...
    if logger:
        if api_key:
            ...
            logger.info("Using Claude CLI authentication (claude auth login)")
        logger.info(f"Claude Code path: {claude_path}")

    # No critical errors - both auth methods are valid
    # If neither works, it will fail during actual execution with clearer error
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    """
    safe_env_vars = {
        # Anthropic Configuration (optional - falls back to claude auth login)
        "ANTHROPIC_API_KEY": os.getenv("ANTHROPIC_API_KEY"),

        # AWS Bedrock Configuration (optional - alternative to Anthropic API)
    ...
    # Explicitly unset CLAUDECODE — if ADW is launched from inside a Claude Code
    # session, this env var is inherited and causes `claude -p` to refuse to run
