...
description: Scaffold a new Adobe Experience PM app under apps/experience-pm/ using Spectrum S2 components from a wireframe/POC spec
argument-hint: [wireframe-poc-spec]
model: opus
...
Scaffold a new React SPA boilerplate for the Adobe Experience PM app under `apps/experience-pm/`, using Adobe Spectrum S2 components. This command reads a wireframe/POC specification, uses the `adobe-spectrum` agent to look up correct S2 component imports, props, and patterns, scaffolds the full directory structure mirroring `apps/experience-qa/client/`, and then invokes `/experts:frontend:plan` to elaborate an implementation plan. Follow the `Instructions` and `Workflow` sections below.

## Variables

WIREFRAME_SPEC: $1
APP_ROOT: apps/experience-pm
CLIENT_ROOT: apps/experience-pm/client

## Instructions

- The new app lives at `apps/experience-pm/` — never modify anything under `apps/experience-qa/`.
...
4. **Create directory structure** — Create the folder tree:
   ...
   └── client/
       ...
       └── src/
           ...
           └── lib/
   ```
5. **Scaffold config files** — Write `package.json` (with `@react-spectrum/s2`, `react-router-dom`, `react`, `vite` deps), `vite.config.ts`, `tsconfig.json`, and `index.html`.
...
9. **Invoke /experts:frontend:plan** — Call the `frontend` expert plan skill to elaborate a full implementation plan based on the wireframe, referencing the newly scaffolded files.

## Report

After completion, report back with:
- A tree listing of all files created under `apps/experience-pm/`
